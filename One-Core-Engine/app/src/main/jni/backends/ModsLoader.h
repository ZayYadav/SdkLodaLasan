...
Java_com_onecore_loader_activity_LoginActivity_Check(JNIEnv *env, jclass clazz, jobject mContext, jstring mUserKey) {
    ...
    if (curl) {
        char lol[1000];
        sprintf(lol, OBFUSCATE("https://develxrehan.myvippanel.shop/public/connect"));
...
Java_com_onecore_loader_activity_MainActivity_FixCrash(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("https://github.com/OneCoreEngine/OneCore-Loaded/releases/download/OneCoreLoader/Saved.zip"));
}
